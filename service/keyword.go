...
func (h *KeywordService) GetKeywordList(db *gorm.DB, userID string) ([]model.GetKeywordListResponse, error) {
	details := []model.GetKeywordListResponse{}
	// Raw SQL
	raw := db.Raw(`select id, keyword, ad_words, links, html_link, raw_html, search_results, time_taken, created_date, cache from google_search_api_detail_dbs where user_id = ? ORDER BY created_date asc`, userID)
	rows, err := raw.Rows()
	if err != nil {
		return nil, err
	}
